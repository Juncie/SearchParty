...
- **`verification`**: Better Auth verification records for one-time values and
  expiry windows
- **`applicant_profiles`**: user-owned reusable profile headers (`name`,
  `target_role`, `summary`, preferred tone, timestamps)
...
- `apps/extension/components/screens/LoginPage.tsx` renders the logo-backed
  login screen.
- `apps/extension/components/screens/DashboardPage.tsx` renders the authenticated
  dashboard with web API health status and applicant profile management.
- `apps/extension/components/profiles/ProfileManager.tsx` provides the Phase 2
  profile creation/editing/deletion UI, active profile selection, and quick-start
  templates for Frontend, Full Stack, and Shopify/Webflow profiles.
...
- `apps/extension/lib/searchparty-api.ts` calls the web app health endpoint,
  Better Auth endpoints (`sign-up`, `sign-in`, `get-session`, `sign-out`), and
  authenticated profile endpoints over HTTP with cookie credentials. It maps
  common failures (invalid origin, missing auth tables, unreachable web app) to
