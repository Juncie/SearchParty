...
import type {AccountOnboardingFieldId} from '@searchparty/data/profile-questions';
 ...
 */
export function partitionOnboardingAnswers(answers: Record<string, unknown>): {
  ...
  for (const [key, value] of Object.entries(answers)) {
    if (
      ACCOUNT_ONBOARDING_FIELD_IDS.includes(key as AccountOnboardingFieldId)
    ) {
      account[key] = value
    } else {
      profile[key] = value
...
export function buildApplicantProfileInputFromAnswers(
  ...
  const { profile: profileAnswers } = partitionOnboardingAnswers(answers)

  const desiredTitles = coerceStringArray(profileAnswers.desiredJobTitles)
  const skillNames = coerceStringArray(profileAnswers.skills)

  const onboardingAnswers: Record<string, unknown> = { ...profileAnswers }
  ...
  const parsed = applicantProfileInputSchema.safeParse({
    name:
      typeof profileAnswers.profileName === 'string'
        ...
        : '',
    targetRole:
      desiredTitles.length > 0 ? desiredTitles[0] ?? '' : 'Open to opportunities',
    summary:
    ...
    lastName:
      typeof profileAnswers.lastName === 'string'
        ...
        : '',
    phone:
      typeof profileAnswers.phone === 'string' ? profileAnswers.phone : '',
    address:
