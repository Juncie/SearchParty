...
/** Login, sign-up, and forgot-password all render under these routes. */
export function shouldShowExtensionBottomNav(
  pathname: string
): boolean {
...
export function BottomNav() {
  ...
  if (!shouldShowExtensionBottomNav(pathname)) {
    return null;
  }
  ...
  return (
    <nav
      aria-label="Primary"
      className="fixed inset-x-0 bottom-0 z-50 flex w-full max-w-none border-t border-border bg-card/95 backdrop-blur-xs pt-1 pb-[max(0.25rem,env(safe-area-inset-bottom,0px))] ring-1 ring-foreground/10"
    >
